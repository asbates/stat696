...
For lasso and random forest we interpret the models via variable importance measures. For variable importance of the lasso we use the absolute value of the regression coefficients. Feature importance for random forest is based on out-of-bag predictions before and after permuting a feature. Importance measures for both models are then scaled so that the most influential feature has an importance of 100. Variable importance measures are easier to assess in a plot, which is given in Figure
