...
nocite: | 
 @broom, @R-car, @R-caret, @R-corrplot, @R-dplyr, @R-ggplot2, @Rglmnet, @R-MASS, @R-purrr, @R-randomForest, @R-recipes, @R-kableExtra
...
```{r corrplot-initial, cache = TRUE, fig.width=6, fig.height=4, fig.cap="Correlation matrix plot of continuous variables prior to transformations. Correlations are displayed as a percent."}
bball %>% 
...
```{r corrplot-logged, cache = TRUE, fig.width = 6, fig.height = 4, fig.cap = "Correlation matrix plot of continuous variables after log transformations. Correlations are displayed as a percent."}
bball_log_only %>% 
  select_if(is.numeric) %>% 
  cor() %>% 
  corrplot(type = "lower",
