...
pred_results <- tibble(
  ...
  `Testing error` = c(lr_test_error, lasso_test_error, rf_test_error)
) %>% 
  mutate(
    `CV relative error` = `CV error` / min(`CV error`),
    `Testing relative error` = `Testing error` / min(`Testing error`)
  ) %>%
  select(Model,
         `CV error`,
         `CV relative error`,
  ...
  arrange(`Testing error`, `CV error`)

pred_results %>% 
  kable(digits = 2, caption = "Prediction results for linear regression, lasso, and random forest. Included are the mean cross-validation RMSE and test set RMSE. Also included are the relative cross-validation and test set error.") %>% 
