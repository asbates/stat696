...
obviously regression summary table and interpretation of that. also include regression coefficients for lasso? will this be too much? but will levine want this? 

Even if the main objective for building a model is prediction accuracy one usually wants to have an understanding of how the model is working. For example, we might be interested in how the covariates affect the response or which covariates are most important to the model.


for regression, mention something about being cautious with inferences because of model selection procedure and cite @sequential-regression.
