...
res_qq <- ny_high %>%
  ...
  stat_qq_line(distribution = qt, dparams = dist_pars[["df"]], color = "#00BFC4") +
...
The projections for Warwick and Monroe in $2005$ and $2025$, along with expenditure predictions and prediction intervals, can be found in Table \ref{tbl:pred}. The most obvious thing to note is the the estimated expenditure for Monroe is the same in $2005$ and $2025$. The increased expenditures from rising population and wealth from $2005$ to $2025$ is probably offset by the decrease in expenditure from rising intergovermental funding rate. The net effect being that the predicted expentures remain the same. Warwick on the other hand, is projected to see larger increases in population, wealth, and growth rate over the twenty year period than Monroe will. This leads to an increase in expenditures over the same period of \$$10$ per person for Warwick. Although Warwick should expect to see their expenditures rise, by $2025$ we expect to see roughly the same expenditure per person for both towns.
